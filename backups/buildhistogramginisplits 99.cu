...
)
{
    ...
    int proj_id = blockIdx.y; // Each block handles one projection
    int row_id = threadIdx.x + blockIdx.x * blockDim.x; // Each thread handles one row
    int base_idx = proj_id * num_rows; // Base index for the current projection
    if (proj_id >= num_proj || row_id >= num_rows) return;

    float min_val = d_min_vals[proj_id];
    float max_val = d_max_vals[proj_id];
    float bin_width = d_bin_widths[proj_id];
    float val = d_col_add_projected[base_idx + row_id];

    int bin;
    //Get bin index per row per projection
    if (val >= max_val) {
        bin = num_bins - 1;
    } else {
        bin = static_cast<int>((val - min_val) / bin_width);
        bin = max(0, min(bin, num_bins - 1));  // Clamp to valid range
    }

    int label = d_labels[row_id];
    if (label == 0) {
        atomicAdd(&d_hist_class0[proj_id * num_bins + bin], 1);
    } else {
        atomicAdd(&d_hist_class1[proj_id * num_bins + bin], 1);
    }
}
